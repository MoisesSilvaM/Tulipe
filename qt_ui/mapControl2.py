...
class FoliumDisplay2(QWidget):
    ...
    def redraw_folium_map(self, geojson_pathO, geojson_pathR, traffic_indicator, p1, p2, p3, p4, minim, maxim, closing):
        ...
        # JavascriptLink('./L.Map.Sync.js'))
        # self.folium_map = folium.Map(zoom_start=self.zoom_level, location=(self.lon, self.lat))
        # self.folium_map.get_root().html.add_child(JavascriptLink('https://github.com/jieter/Leaflet.Sync/L.Map.Sync.js'))
    ...
    def __init__(self, geojson_path):
        ...
        #    'C:\\Users\moise\PycharmProjects\pythonProjectDavide\Sumo\osm.net.xml\osm.net.xml')  # os.environ['BXL_NET'])
    ...
    def add_geojson_map1(self, map, fname, traffic_indicator, p1, p2, p3, p4, minim, maxim):
        ...
        with open(fname) as fO:
            dataO = json.load(fO)
        popup = folium.GeoJsonPopup(fields=["id", "{}".format(traffic_indicator)])
        ...
        linear1.caption = traffic_indicator
        # if traffic_indicator == "edge_speed" or traffic_indicator == "edge_speedRelative":
        #     step.colors.reverse()
        #     # step.index.reverse()
        ...
        elem = folium.GeoJson(dataO,
                              ...
                              style_function=lambda feature: {
                                  "color": step(feature['properties']['{}'.format(traffic_indicator)]),
                                  "weight": 4
                                  # "dashArray": "5, 5",
                              }
                              )
    ...
    def add_geojson_map2(self, map, fname, traffic_indicator, p1, p2, p3, p4, minim, maxim, closing):
        ...
        elem = folium.GeoJson(dataR,
                              ...
                              popup_keep_highlighted=True,
                              style_function=lambda feature: {
                                  "color": step(feature['properties']['{}'.format(traffic_indicator)]) if feature['properties']['id'] not in closing else "#00FFF7",
                                  "weight": 4
                                  # "dashArray": "5, 5",
                              }
                              )
        ...
        #folium.TileLayer('cartodbpositron').add_to(map.m2)
            ...
            #step.index.reverse()
            #linear1.colors.reverse()
        # linear2 = cm.LinearColormap(["#0F9D58", "#FBBB05", "#E94335", "#822F2B"], vmin=minim, vmax=maxim, tick_labels=[minim, maxim])
        # linear2.caption = traffic_indicator
        # map.m2.add_child(linear2)
        # ##https://github.com/python-visualization/branca/issues/91
        # svg_style = '<style>svg#legend {background-color: white;}</style>'
        # map.m2.get_root().header.add_child(folium.Element(svg_style))
        #
        elem.add_to(map.m2)
        self.elem2 = elem
        return map
...
"""
    ...
    def map_to_geojson(self, edgedata_out_csv, output_geojson_path, interval, traffic_indicator):
        ...
        #"python \"" + os.environ["SUMO_TOOL"] + "net\\net2geojson.py\" -n {} -o {}".format("C:\\Users\moise\PycharmProjects\pythonProjectDavide\Sumo\osm.net.xml\osm.net.xml", tempf_geojson)
        #print(net2geojson_command)
        ...
        p1 = df3[traffic_indicator].quantile(0.25)
        p2 = df3[traffic_indicator].quantile(0.5)
        p3 = df3[traffic_indicator].quantile(0.75)
        p4 = df3[traffic_indicator].quantile(0.9)
        minim = df3[traffic_indicator].min()
        maxim = df3[traffic_indicator].max()
        df3.to_file(output_geojson_path)
        # print(edgedata_out_csv)
        # print(minim)
        # print(p1)
        # print(p2)
        # print(p3)
